...
contract PoolModifyPositionTest is ILockCallback {
    ...
    struct CallbackData {
        ...
        bytes crosschainParams;
    }

    function modifyPosition(PoolKey memory key, IPoolManager.ModifyPositionParams memory params, bytes memory crosschainParams)
        external
        payable
        returns (BalanceDelta delta)
    {
        delta = abi.decode(manager.lock(abi.encode(CallbackData(msg.sender, key, params, crosschainParams))), (BalanceDelta));
    ...
    function lockAcquired(bytes calldata rawData) external returns (bytes memory) {
        ...
        BalanceDelta delta = manager.modifyPosition(data.key, data.params, data.crosschainParams);
